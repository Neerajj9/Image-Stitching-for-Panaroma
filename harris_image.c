...
image structure_matrix(image im, float sigma)
{
    ...
    image Ix = convolve_image(im, gradient_x, 0);
    image Iy = convolve_image(im, gradient_y, 0);
    float pixel_val;
    for(int i = 0 ; i < S.w ; i++)
    {
        for(int j = 0 ; j < S.h ; j++)
        {
            for(int k = 0 ; k < S.c ; k++)
            {
                if(k == 0)
                {
                    pixel_val = pow(get_pixel(Ix , i , j , 0) , 2);
                    set_pixel(S , i , j , k , pixel_val );
                }
                else if(k == 1)
                {
                    pixel_val = pow(get_pixel(Iy , i , j , 0) , 2);
                    set_pixel(S , i , j , k , pixel_val );
                }
                else
                {
                    pixel_val = get_pixel(Ix , i , j , 0) * get_pixel(Iy , i, j , 0);
                    set_pixel(S , i , j , k , pixel_val );
                }
...
image cornerness_response(image S)
{
    ...
    for(int i = 0 ; i < S.w ; i++)
    {
        for(int j = 0 ; j < S.h ; j++)
        {
            ...
            float trace = Ixx + Iyy;
            float cornerness = det - 0.6*pow(trace,2);
